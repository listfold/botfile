...
// always a kind directory (plural: skills/, memories/), and the third is the
// component: a skill is a directory containing SKILL.md, a memory is a
...
// memoryExt is the extension of a memory file (manifesto 18, 48).
const memoryExt = ".md"
...
// 47): KindSkill -> "skills", KindMemory -> "memories". The projection layer
// uses it (with ComponentLeaf) to reconstruct a component's path under a source.
...
func MemoryName(entry string) (string, bool) {
	...
	return strings.TrimSuffix(entry, memoryExt), true
}
